...
// セレクトクラスのインスタンス生成
//=============================================================================
CSelect * CSelect::Create(D3DXVECTOR3 pos, float fSizeX, float fSizeY, SELECTTYPE SelectType)
{
	// CTextのポインタ
	CSelect *pText = NULL;
	...
	{
		// 座標やサイズのセット
		pText->SetText(pos, fSizeX, fSizeY, SelectType);
...
HRESULT CSelect::Init()
{
	...
	{
		// テキストクラスのクリエイトと色のセット
		m_pText[0] = m_pText[0]->Create(D3DXVECTOR3(m_pos.x, m_pos.y - 100.0f, m_pos.z), 662.0f, 150.0f, CText::TEXTTYPE_RESTART);
		m_pText[0]->SetColor(255, 255, 255, 255);
		m_pText[1] = m_pText[1]->Create(D3DXVECTOR3(m_pos.x, m_pos.y + 100.0f, m_pos.z), 695.0f, 132.0f, CText::TEXTTYPE_MODORU);
	...
	if (m_pSelectType == SELECTTYPE_MENU)
	{
		...
		if (m_nCount == 0)
		{
			...
			if (m_bButtonPress == false)
			{
				...
				{
					// ガッチャンのクリエイト
					CGattyann::Create(D3DXVECTOR3(SCREEN_CENTER_X / 4, SCREEN_HEIGHT + SCREEN_CENTER_Y, 0.0f), 432.0f, 1080.0f, CGattyann::GATTYANN_5);
					CGattyann::Create(D3DXVECTOR3(SCREEN_CENTER_X - SCREEN_CENTER_X / 4, -SCREEN_CENTER_Y, 0.0f), 432.0f, 1080.0f, CGattyann::GATTYANN_6);
					CGattyann::Create(D3DXVECTOR3(SCREEN_CENTER_X + SCREEN_CENTER_X / 4, SCREEN_HEIGHT + SCREEN_CENTER_Y, 0.0f), 432.0f, 1080.0f, CGattyann::GATTYANN_7);
					CGattyann::Create(D3DXVECTOR3(SCREEN_WIDTH - SCREEN_CENTER_X / 4, -SCREEN_CENTER_Y, 0.0f), 432.0f, 1080.0f, CGattyann::GATTYANN_8);

...
HRESULT CExplosion::Load(void)
{
	...
	// テクスチャの読み込み
	D3DXCreateTextureFromFile(pDevice, TEXTURE_EXPLOSION, &m_pTexture[EXPLOSIONTYPE_NOMAL]);	   // 弾の爆発
	D3DXCreateTextureFromFile(pDevice, TEXTURE_BOSSEXPLOSION, &m_pTexture[EXPLOSIONTYPE_BOSS]); // ボスアタックの爆発
...
HRESULT CExplosion::Init()
{
	...
	case EXPLOSIONTYPE_NOMAL:
		// テクスチャの設定
		BindTexture(m_pTexture[EXPLOSIONTYPE_NOMAL]);
	...
	case EXPLOSIONTYPE_BOSS:
		// テクスチャの設定
		BindTexture(m_pTexture[EXPLOSIONTYPE_BOSS]);

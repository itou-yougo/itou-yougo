...
#define OJAMA_TYPE_FALL_MIDDLE (D3DXVECTOR3(SCREEN_CENTER_X, 50.0f, 0.0f))
#define OJAMA_TYPE_FALL_RANGE (400)
#define OJAMA_TYPE_FALL_RANGE_SUBTRACT (200)

#define OJAMA_TYPE_CURVE_MIDDLE (D3DXVECTOR3(SCREEN_CENTER_X, (float)(rand() % 455) + 125.0f, 0.0f))
#define OJAMA_TYPE_CURVE_MOVE (10.0f)

#define EFFECT_ADD_SIZE (D3DXVECTOR3(5.0f, 5.0f, 0.0f))
#define EFFECT_ALIFE (10)

#define OJAMA_MOVE (D3DXVECTOR3(-10.0f, -10.0f, 0.0f))
#define OJAMA_MOVE_OBLIQUE (D3DXVECTOR3(15.0f, 15.0f, 0.0f))
#define OJAMA_SIZE (D3DXVECTOR3(50.0f, 50.0f, 0.0f))
#define OJAMA_ACCEL (D3DXVECTOR3(2.0f, 2.0f, 0.0f))
#define OJAMA_ACCEL_OBLIQUE (D3DXVECTOR3(5.0f, 5.0f, 0.0f))
...
HRESULT COjama::Init()
{
	...
	case OJAMAPLAYER_1P:
		...
		case OJAMA_TYPE_FALL:
			// 中心と角度の代入
			m_middle = OJAMA_TYPE_FALL_MIDDLE;
		...
		case OJAMA_TYPE_CURVE:
			// 中心の代入
			m_middle = OJAMA_TYPE_CURVE_MIDDLE;
			break;
...
void COjama::Update(void)
{
	...
	case OJAMAPLAYER_1P:
		// エフェクトのクリエイト
		CEffect::Create(m_pos, m_size + EFFECT_ADD_SIZE, EFFECT_ALIFE, 255, 100, 100, 255, CEffect::EFFECTTYPE_BULLET, OBJTYPE_EFFECT);
		break;

		// 2Pのおじゃま
	case OJAMAPLAYER_2P:	// エフェクトのクリエイト
		CEffect::Create(m_pos, m_size + EFFECT_ADD_SIZE, EFFECT_ALIFE, 100, 255, 255, 255, CEffect::EFFECTTYPE_BULLET, OBJTYPE_EFFECT);
		break;
...
void COjama::Curve(void)
{
	...
	case OJAMAPLAYER_1P:
		...
		if (m_pos.x < m_middle.x)
		{
			...
			// 決めた角度の方に動かす
			m_move.x = sinf(m_fAngle) * -OJAMA_TYPE_CURVE_MOVE;
			m_move.y = cosf(m_fAngle) * -OJAMA_TYPE_CURVE_MOVE;
		}
		// 中心より大きいとき
...
void COjama::Fall(void)
{
	...
	case OJAMAPLAYER_1P:
		...
		if (m_middle.y >= m_pos.y)
		{
			...
			// 落ちる座標を決める
			m_Fallpos.x = (float)(rand() % OJAMA_TYPE_FALL_RANGE - OJAMA_TYPE_FALL_RANGE_SUBTRACT) + m_Getpos.x;
	...
	if (((CBullet*)pScene)->GetBulletType() == CBullet::BULLET_TYPE_1P)
	{
		if (m_OjamaPlayerNum == OJAMAPLAYER_2P)
		{
			...
			if (m_nCount < 1)
			{
				...
				{
				case 0:
					COjama::Create(m_pos, OJAMA_MOVE, OJAMA_SIZE,
						m_nCount + 1, OJAMA_ACCEL, COjama::OJAMA_TYPE_THROUGH, COjama::OJAMAPLAYER_1P, OBJTYPE_OJAMA);
					break;

				case 1:
					COjama::Create(m_pos, OJAMA_MOVE, OJAMA_SIZE,
						m_nCount + 1, D3DXVECTOR3(-(float)(rand() % 3) - OJAMA_ACCEL.x, -OJAMA_ACCEL.y, 0.0f), COjama::OJAMA_TYPE_CURVE, COjama::OJAMAPLAYER_1P, OBJTYPE_OJAMA);
					break;

				case 2:
					COjama::Create(m_pos, D3DXVECTOR3(-(float)(rand() % 10) + OJAMA_MOVE_OBLIQUE.x, -(float)(rand() % 10) + OJAMA_MOVE_OBLIQUE.y, 0.0f),
						OJAMA_SIZE, m_nCount + 1,
						D3DXVECTOR3(-(float)(rand() % 5) - OJAMA_ACCEL_OBLIQUE.x, OJAMA_ACCEL_OBLIQUE.y, 0.0f), COjama::OJAMA_TYPE_FALL, COjama::OJAMAPLAYER_1P, OBJTYPE_OJAMA);
					break;
				default:
					break;
				}
	...
	if (((CBullet*)pScene)->GetBulletType() == CBullet::BULLET_TYPE_2P)
	{
		if (m_OjamaPlayerNum == OJAMAPLAYER_1P)
		{
			...
			if (m_nCount < 1)
			{
				...
				case 0:
					COjama::Create(m_pos, OJAMA_MOVE, OJAMA_SIZE,
						m_nCount + 1, OJAMA_ACCEL, COjama::OJAMA_TYPE_THROUGH, COjama::OJAMAPLAYER_2P, OBJTYPE_OJAMA);
					break;

				case 1:
					COjama::Create(m_pos, OJAMA_MOVE, OJAMA_SIZE,
						m_nCount + 1, D3DXVECTOR3(-(float)(rand() % 3) - OJAMA_ACCEL.x, -OJAMA_ACCEL.y, 0.0f), COjama::OJAMA_TYPE_CURVE, COjama::OJAMAPLAYER_2P, OBJTYPE_OJAMA);
					break;

				case 2:
					COjama::Create(m_pos, D3DXVECTOR3(-(float)(rand() % 10) + OJAMA_MOVE_OBLIQUE.x, -(float)(rand() % 10) + OJAMA_MOVE_OBLIQUE.y, 0.0f),
						OJAMA_SIZE, m_nCount + 1,
						D3DXVECTOR3(-(float)(rand() % 5) - OJAMA_ACCEL_OBLIQUE.x, OJAMA_ACCEL_OBLIQUE.y, 0.0f), COjama::OJAMA_TYPE_FALL, COjama::OJAMAPLAYER_2P, OBJTYPE_OJAMA);
					break;
